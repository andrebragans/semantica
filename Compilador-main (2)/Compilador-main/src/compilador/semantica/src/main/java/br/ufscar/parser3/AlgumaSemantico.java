...
public class AlgumaSemantico extends AlgumaBaseVisitor<Void>{
    ...
    public Void visitDeclaracao(AlgumaParser.DeclaracaoContext ctx) {
       String = ctx.VARIAVEL().getText();
       String strTipoVar = ctx.TIPO_VAR().getText();
       TipoAlguma tipaVar = TipoAlguma.INVALIDO;

       switch(strTipoVar) {
         case "INTEIRO": tipoVar = TipoAlguma.INTEIRO; break;
         case "REAL": tipoVar = TipoAlguma.REAL; break;
       }
    
       if (tabela.existe(nomeVar)) {
           AlgumaSemanticoUtiIs.adicionarErroSemantico(ctx.VARIAVEL().getSymbol(),
    ...
    public Void visitComandoAtribuicao(AlgumaParser.ComandoAtribuicaoContext ctx) {
      TipoAlguma tipoExpressao = AlgumaSemanticoUtils.verificarTipo(tabela, ctx.expressoes);
        if(tipoExpressao != TipoAlguma.INVALIDO) {
        String nomeVar  = ctx.VARIAVEL().getText();
        if (!tabela.existe(nomeVar)) {
           AlgumaSemanticoUtils.adicionarErroSemantico(ctx.VARIAVEL().getSymbol(),"Variavel"+nomeVar+"não foi declarada antes do uso");
        } else {
        TipoAlguma tipoVariavel = AlgumaSemanticoUtils.verificarTipo(tabela, nomeVar);
        if(tipoVariavel != tipoExpressao) {
        AlgumaSemanticoUtils.adicionarErroSemantico(ctx.VARIAVEL().getSymbol(), "Tipo da variavel "+nomeVar+ "não é compativel com o tipo da expressão");
        }
    }
}
    ...
    public Void visitComandoEntrada(AlgumaParser.ComandoEntradaContext ctx) {
        String nomeVar = ctx.VARIAVEL().getText();
        if (!tabela.existe(nomeVar)) {
        AlgumaSemanticoUtiIs.adicionarErroSemantico(ctx.VARIAVEL().getSymbol(), "Variavel " +nomeVar+ "não foi declarada antes do uso");
        }
